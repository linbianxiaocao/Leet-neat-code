# http://massivealgorithms.blogspot.com/2017/07/hilbert-curve-airbnb.html

int hilbertCurve(int x, int y, int iter) {
    if (iter == 0) return 1;
    int areaCnt = (1 << (iter * 2 - 2)); // 每一块区域边长的边界值, (当前iter下每个区域的点数）
    int borderLen = (1 << (iter - 1)); // 区域移动的长度(每个小cell区域的宽度）
